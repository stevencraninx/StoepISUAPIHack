...
async function loadScheduleFile(dayParam) {
  const day = dayParam || new URLSearchParams(window.location.search).get("day") || "wc_day1";
  try {
    ...
    try {
      const resp = await fetch(url, {
        ...
        headers: {
          ...
          "Referer": `https://isu-skating.com/short-track/results/isu-world-short-track-speed-skating-championships-2/${event_result_id}/`
        }
      });
...
async function loadSchedule(dayParam) {
  ...
  const day = dayParam || params.get("day") || "wc_day1";
